...
- **Cold recovery: `read` must not rely on the persisted physical name.** Physical names from `createPhysicalName` embed a random per-instance suffix that lives only in alchemy state — after state loss, probing a freshly-generated name silently misses and the engine plans a duplicate `create` (a second Cloud SQL instance / 72 TiB Lustre / node pool). When `read` is called with no persisted name (`output`/`olds` both lack it), fall back to *listing* the parent scope and matching `hasAlchemyLabels` (SqlInstance scans `settings.userLabels`, ManagedLustreInstance scans `labels`, NodePool scans `config.resourceLabels`). New resources with generated names must follow this pattern. The engine's plan-time adoption probe runs read-recovery one dependency layer per `plan` invocation (props must resolve from upstream state first), so a full cold rebuild of state takes several consecutive `alchemy plan` runs.
